  ...
  if(isset($_GET['p'])){
    switch ($_GET['p']) {
      ...
      case 6: // โปรแกรมแจกแจงเงินเป็นธนบัตร/เหรียญ
        $code = '<div>/* โปรแกรมแจกแจงเงินเป็นธนบัตร/เหรียญ */</div><div><br></div><div>ประกาศ save ,money, rem,&nbsp;<span>th=0, hTh</span><span>=0</span><span>, h</span><span>=0</span><span>, hH</span><span>=0</span><span>, tw</span><span>=0</span><span>, t</span><span>=0</span><span>, f</span><span>=0</span><span>, d</span><span>=0</span><span>; //1000 500 100 50 20 10 5 2</span></div><div>money = รับ("กรุณาใส่จำนวนเงิน");</div><div>save = money;</div><div>ถ้า(money&gt;1000-1){</div><div>rem = money%1000;</div><div>money -= rem;</div><div>th = money/1000;</div><div>money /= 1000;</div><div>money = money + rem - th;</div><div>}</div><div><div>ถ้า(money&gt;500-1){</div><div>rem = money%500;</div><div>money -= rem;</div><div>hTh = money/500;</div><div>money /= 500;</div><div>money = money + rem - hTh;</div><div>}</div></div><div><div>ถ้า(money&gt;100-1){</div><div>rem = money%100;</div><div>money -= rem;</div><div>h = money/100;</div><div>money /= 100;</div><div>money = money + rem - h;</div><div>}</div><div><div>ถ้า(money&gt;50-1){</div><div>rem = money%50;</div><div>money -= rem;</div><div>hH = money/50;</div><div>money /= 50;</div><div>money = money + rem - hH;</div><div>}</div><div><div>ถ้า(money&gt;20-1){</div><div>rem = money%20;</div><div>money -= rem;</div><div>tw = money/20;</div><div>money /= 20;</div><div>money = money + rem - tw;</div><div>}</div><div><div>ถ้า(money&gt;10-1){</div><div>rem = money%10;</div><div>money -= rem;</div><div>t = money/10;</div><div>money /= 10;</div><div>money = money + rem - t;</div><div>}</div><div><div>ถ้า(money&gt;5-1){</div><div>rem = money%5;</div><div>money -= rem;</div><div>f = money/5;</div><div>money /= 5;</div><div>money = money + rem - f;</div><div>}</div><div><div>ถ้า(money&gt;2-1){</div><div>rem = money%2;</div><div>money -= rem;</div><div>d = money/2;</div><div>money /= 2;</div><div>money = money + rem - d;</div><div>}</div><div>แสดง("จำนวนเงินทั้งหมด "+save+" บาท ประกอบด้วย\n &nbsp; - ธนบัตร 1000 บาท "+th+" ใบ\n &nbsp; - ธนบัตร 500 บาท "+hTh+" ใบ\n &nbsp; - ธนบัตร 100 บาท "+h+" ใบ\n &nbsp; - ธนบัตร 50 บาท "+hH+" ใบ\n &nbsp; - ธนบัตร 20 บาท "+tw+" ใบ\n &nbsp; - เหรียญ 10 บาท "+t+" เหรียญ\n &nbsp; - เหรียญ 5 บาท "+f+" เหรียญ\n &nbsp; - เหรียญ 2 บาท "+d+" เหรียญ\n &nbsp; - เหรียญ 1 บาท "+money+" เหรียญ");</div></div></div></div></div></div></div>';
        break;
      default:
        $code = "";
        break;
